...
class AdminController extends Controller
{
    ...
    public function showCenter($id) {
        ...
        // Eager load pair evaluations
        $center = DiscoveryCenter::with(['users', 'notes.author', 'notes.taggedUser', 'pairEvaluations'])->findOrFail($id);
    ...
    public function showPair($centerId, $pairName) {
        ...
        if ($user->role === 'participant') {
            if ($user->center_id != $centerId || $user->pair_name != $pairName) {
                abort(403, 'No tienes permiso para ver esta pareja.');
            }
        }

        $center = DiscoveryCenter::findOrFail($centerId);
        $users = User::where('center_id', $centerId)->where('pair_name', $pairName)->with('evaluationItems')->get();
        ...
        $pairEvaluation = PairEvaluation::where('center_id', $centerId)->where('pair_name', $pairName)->first();

        // Check if decision is visible for participants
        $showDecision = true;
        if ($user->role === 'participant' && $pairEvaluation) {
            if (!$pairEvaluation->visible_at || $pairEvaluation->visible_at->isFuture()) {
                $showDecision = false;
            }
        }
        ...
        return view('admin.pair_details', [
            'center' => $center,
            'pairName' => $pairName,
            'husband' => $husband,
            'wife' => $wife,
    ...
    public function addEvaluationItem(Request $request) {
        EvaluationItem::create($request->validate([
            'user_id' => 'required|exists:users,id',
            'type' => 'required|in:strength,growth,suggestion',
            'content' => 'required|string'
        ]));
        return back()->with('success', 'Elemento de evaluación agregado.');
    }

    public function updateEvaluationItem(Request $request, $id) {
        $item = EvaluationItem::findOrFail($id);
        $item->update($request->validate(['content' => 'required|string']));
        return back()->with('success', 'Elemento actualizado.');
    }

    public function deleteEvaluationItem($id) {
        EvaluationItem::findOrFail($id)->delete();
        return back()->with('success', 'Elemento eliminado.');
    }

    public function setPairDecision(Request $request) {
        $data = $request->validate([
            'center_id' => 'required',
            'pair_name' => 'required',
            'decision' => 'required|in:green,yellow,red',
            'decision_text' => 'required|string',
            'visible_at' => 'nullable|date'
        ]);

        PairEvaluation::updateOrCreate(
            ['center_id' => $data['center_id'], 'pair_name' => $data['pair_name']],
            ['decision' => $data['decision'], 'decision_text' => $data['decision_text'], 'visible_at' => $data['visible_at']]
        );

        return back()->with('success', 'Decisión final actualizada.');
    }

    public function generatePairPdf($userId) {
        $user = User::with(['center', 'evaluationItems'])->findOrFail($userId);
        $centerId = $user->center_id;
        $pairName = $user->pair_name;

        $pairUsers = User::where('center_id', $centerId)->where('pair_name', $pairName)->with('evaluationItems')->get();
        $husband = $pairUsers->first();
        $wife = $pairUsers->count() > 1 ? $pairUsers->last() : null;

        $pairEvaluation = PairEvaluation::where('center_id', $centerId)->where('pair_name', $pairName)->first();

        $pdf = Pdf::loadView('pdf.pair_cover', compact('user', 'husband', 'wife', 'pairEvaluation'))
            ->setPaper('letter', 'portrait');

        return $pdf->stream("expediente_{$pairName}.pdf");
    }
    ...
    public function printResult($id) {
        ...
        $mbtiInfo = $this->getMbtiInfo();
        $donesInfo = $this->getDonesInfo();
        return Pdf::loadView('pdf.test_result', compact('result', 'mbtiInfo', 'donesInfo'))->setPaper('letter', 'portrait')->stream();
    }
    ...
    public function takeTest($userId, $type) {
        ...
        return view('admin.take_test', ['user' => $user, 'type' => $type, 'questions' => $questions, 'timer' => $user->center->quiz_timer ?? 15]);
    }
    ...
    public function saveTest(Request $request) {
        ...
        return redirect()->route('pairs.show', [$user->center_id, $user->pair_name])->with('success', 'Test guardado.');
    }

    private function getEnrichedResults($userName, $pairName) {
        return TestResult::where('user_name', 'like', trim($userName))
            ->where('pair_name', $pairName)
            ->orderBy('completed_at', 'desc')
            ->get()
            ->map(fn($res) => $this->enrichSingleResult($res));
    }
    ...
    private function enrichSingleResult($res) {
        ...
        if ($res->test_type == 'dones') {
            ...
            foreach ($ans as $index => $item) { $scores[$codes[$index % 14]] += (int)($item['answer'] ?? 0); }
            $ranking = [];
            foreach ($scores as $code => $score) { $ranking[] = ['code' => $code, 'name' => $names[$code], 'score' => $score]; }
        ...
        if ($res->test_type == 'mbti') {
            ...
            foreach ($ans as $item) {
                $idx = ($item['number'] - 1) % 8;
                if (isset($dims[$idx])) { $scores[$dims[$idx]] += (int)$item['answer']; }
            }
            $meta['scores'] = $scores;
    ...
    private function getDonesQuestions() {
        return [
            ...
            ["number" => 57, "text" => "Soy bueno coordinando el trabajo de many voluntarios."],
            ["number" => 58, "text" => "Juzgo las enseñanzas según si están alineadas con la Biblia."],
            ["number" => 59, "text" => "Me siente impulsado a invitar a personas a la iglesia."],
            ...
            ["number" => 84, "text" => "Suelo ver la solution lógica a problemas espirituales."],
            ...
            ["number" => 95, "text" => "Mis palabras a menudo traen conconvicción de pecado."],
    ...
    public function getMbtiInfo() {
        return [
            "ISTJ" => ["name" => "El Inspector", "description" => "Personas reservadas, prácticas y silenciosas.", "strengths" => ["Responsables", "Organizados", "Leales", "Detallistas"], "weaknesses" => ["Rígidos", "Poco empáticos", "Resistentes al cambio"]],
            "ISFJ" => ["name" => "El Protector", "description" => "Convencionales y con los pies en la tierra.", "strengths" => ["Serviciales", "Confiables", "Observadores", "Prácticos"], "weaknesses" => ["Demasiado altruistas", "Se toman las cosas personalmente", "Reprimen sentimientos"]],
            "INFJ" => ["name" => "El Consejero", "description" => "Idealistas que tienen un gran sentido de la integridad personal.", "strengths" => ["Perspicaces", "Inspiradores", "Decididos", "Apasionados"], "weaknesses" => ["Sensibles a la crítica", "Extremadamente reservados", "Agotamiento"]],
            "INTJ" => ["name" => "La Mente Maestra", "description" => "Analíticos, lógicos y creativos.", "strengths" => ["Estratégicos", "Independientes", "Analíticos", "Determinados"], "weaknesses" => ["Arrogantes", "Demasiado críticos", "Desconectados"]],
            "ISTP" => ["name" => "El Artesano", "description" => "Prácticos y realistas.", "strengths" => ["Optimistas", "Creativos", "Prácticos", "Relajados"], "weaknesses" => ["Tercos", "Insensibles", "Reservados"]],
            "ISFP" => ["name" => "El Compositor", "description" => "Artísticos, sensibles y amables.", "strengths" => ["Encantadores", "Sensibles", "Imaginativos", "Apasionados"], "weaknesses" => ["Independientes en exceso", "Impredecibles", "Estresables"]],
            "INFP" => ["name" => "El Sanador", "description" => "Sensibles, idealistas y leales.", "strengths" => ["Idealistas", "Buscadores de armonía", "Mente abierta", "Creativos"], "weaknesses" => ["Demasiado idealistas", "Demasiado altruistas", "Se lo toman personal"]],
            "INTP" => ["name" => "El Arquitecto", "description" => "Lógicos, precisos y reservados.", "strengths" => ["Objetivos", "Imaginativos", "Entusiastas", "Grandes pensadores"], "weaknesses" => ["Desconectados", "Insensibles", "Dudosos"]],
            "ESTP" => ["name" => "El Promotor", "description" => "Enérgicos y orientados a la acción.", "strengths" => ["Audaces", "Directos", "Sociables", "Perspicaces"], "weaknesses" => ["Insensibles", "Impacientes", "Arriesgados"]],
            "ESFP" => ["name" => "El Actor", "description" => "Amantes de la diversión, sociables y entusiastas.", "strengths" => ["Audaces", "Originales", "Estéticos", "Prácticos"], "weaknesses" => ["Sensibles", "Evitan conflictos", "Aburrimiento rápido"]],
            "ENFP" => ["name" => "El Campeón", "description" => "Entusiastas, creativos e imaginativos.", "strengths" => ["Curiosos", "Observadores", "Enérgicos", "Populares"], "weaknesses" => ["Falta de enfoque", "Pensamiento excesivo", "Muy emocionales"]],
            "ENTP" => ["name" => "El Inventor", "description" => "Rápidos, ingeniosos y estimulantes.", "strengths" => ["Conocedores", "Pensadores rápidos", "Originales", "Carismáticos"], "weaknesses" => ["Argumentativos", "Insensibles", "Intolerantes"]],
            "ESTJ" => ["name" => "El Supervisor", "description" => "Prácticos, realistas y orientados a los hechos.", "strengths" => ["Dedicados", "Voluntad fuerte", "Directos", "Organizadores"], "weaknesses" => ["Inflexibles", "Incómodos con lo nuevo", "Demasiado críticos"]],
            "ESFJ" => ["name" => "El Profesor", "description" => "Cooperativos, sociables y de buen corazón.", "strengths" => ["Sentido del deber", "Leales", "Sensibles", "Buenos conectores"], "weaknesses" => ["Preocupados por estatus", "Inflexibles", "Vulnerables"]],
            "ENFJ" => ["name" => "El Profesor", "description" => "Cálidos, empáticos y responsables.", "strengths" => ["Tolerantes", "Confiables", "Carismáticos", "Altruistas"], "weaknesses" => ["Demasiado idealistas", "Desinteresados en exceso", "Muy sensibles"]],
            "ENTJ" => ["name" => "El Mariscal de Campo", "description" => "Francos, decididos y líderes naturales.", "strengths" => ["Eficientes", "Enérgicos", "Confiados", "Voluntad fuerte"], "weaknesses" => ["Tercos", "Intolerantes", "Impacientes", "Arrogantes"]],
        ];
    }
    ...
    public function deletePair(Request $request) {
        ...
        foreach ($users as $user) { if ($user->pair_photo) Storage::disk('public')->delete($user->pair_photo); $user->delete(); }
    ...
    public function addPair(Request $request) {
        $request->validate(['pair_name' => 'required', 'husband_name' => 'required', 'husband_username' => 'required|unique:users,username', 'husband_password' => 'required', 'wife_name' => 'required', 'wife_username' => 'required|unique:users,username', 'wife_password' => 'required', 'pair_photo' => 'nullable|image']);
        $photoPath = $request->hasFile('pair_photo') ? $request->file('pair_photo')->store('pairs', 'public') : null;
        User::create(['name' => $request->husband_name, 'username' => $request->husband_username, 'password' => Hash::make($request->husband_password), 'role' => 'participant', 'pair_name' => $request->pair_name, 'center_id' => $request->center_id, 'pair_photo' => $photoPath]);
        User::create(['name' => $request->wife_name, 'username' => $request->wife_username, 'password' => Hash::make($request->wife_password), 'role' => 'participant', 'pair_name' => $request->pair_name, 'center_id' => $request->center_id, 'pair_photo' => $photoPath]);
    ...
    public function updatePair(Request $request) {
        $husband = User::findOrFail($request->husband_id); $wife = User::findOrFail($request->wife_id);
        $photoPath = $husband->pair_photo;
        if ($request->hasFile('pair_photo')) { if ($photoPath) Storage::disk('public')->delete($photoPath); $photoPath = $request->file('pair_photo')->store('pairs', 'public'); }
        $husband->update(['name' => $request->husband_name, 'username' => $request->husband_username, 'pair_name' => $request->pair_name, 'pair_photo' => $photoPath]);
        if ($request->husband_password) $husband->update(['password' => Hash::make($request->husband_password)]);
        $wife->update(['name' => $request->wife_name, 'username' => $request->wife_username, 'pair_name' => $request->pair_name, 'pair_photo' => $photoPath]);
        if ($request->wife_password) $wife->update(['password' => Hash::make($request->wife_password)]);
        return redirect()->route('centers.show', $request->center_id . '#tab-pairs')->with('success', 'Datos actualizados.');
    }

    public function addStaff(Request $request) {
        User::create(['name' => $request->name, 'username' => $request->username, 'password' => Hash::make($request->password), 'role' => 'admin', 'staff_title' => $request->staff_title, 'center_id' => $request->center_id]);
        return redirect()->route('centers.show', $request->center_id . '#tab-staff')->with('success', 'Staff agregado.');
    }

    public function updateStaff(Request $request, $id) {
        $staff = User::findOrFail($id); $staff->update(['name' => $request->name, 'username' => $request->username, 'staff_title' => $request->staff_title]);
        if ($request->password) $staff->update(['password' => Hash::make($request->password)]);
        return redirect()->route('centers.show', $staff->center_id . '#tab-staff')->with('success', 'Staff actualizado.');
    }

    public function deleteStaff($id) { $staff = User::findOrFail($id); $cId = $staff->center_id; $staff->delete(); return redirect()->route('centers.show', $cId . '#tab-staff')->with('success', 'Staff eliminado.'); }

    public function addNote(Request $request) {
        Note::create(['author_id' => Auth::id(), 'center_id' => $request->center_id, 'content' => $request->content, 'is_public' => $request->has('is_public'), 'tagged_user_id' => $request->tagged_user_id, 'tagged_pair_name' => $request->tagged_pair_name]);
        return redirect()->route('centers.show', $request->center_id . '#tab-notes')->with('success', 'Nota guardada.');
    }

    public function updateNote(Request $request, $id) {
        $note = Note::findOrFail($id); $note->update(['content' => $request->content, 'is_public' => $request->has('is_public'), 'tagged_user_id' => $request->tagged_user_id, 'tagged_pair_name' => $request->tagged_pair_name]);
        return redirect()->route('centers.show', $note->center_id . '#tab-notes')->with('success', 'Nota actualizada.');
    }

    public function deleteNote($id) { $note = Note::findOrFail($id); $cId = $note->center_id; $note->delete(); return redirect()->route('centers.show', $cId . '#tab-notes')->with('success', 'Nota eliminada.'); }

    public function updateAdmin(Request $request, $id) { $admin = User::findOrFail($id); $admin->update(['name' => $request->name, 'username' => $request->username]); if ($request->password) $admin->update(['password' => Hash::make($request->password)]); return back()->with('success', 'Admin actualizado.'); }
    public function deleteAdmin($id) { User::findOrFail($id)->delete(); return back()->with('success', 'Admin eliminado.'); }

    public function printCenterPairs($centerId) {
        $center = DiscoveryCenter::with('users')->findOrFail($centerId); $pairs = $center->users->where('role', 'participant')->groupBy('pair_name');
        return Pdf::loadView('pdf.center_pairs_grid', compact('center', 'pairs'))->setPaper('letter', 'portrait')->stream();
    }
}
